document.addEventListener("DOMContentLoaded", () => {
  ...
  // FX
  const FX = {
    root: document.getElementById('fx'),
  ...
  function rgbSplitPulse(){ const a=document.querySelector('.screen.active'); if(!a) return; a.classList.add('rgb-split'); setTimeout(()=>a.classList.remove('rgb-split'),300); }
  function screenTearOnce(){ const a=document.querySelector('.screen.active'); if(!a) return; a.classList.add('screen-tear'); setTimeout(()=>a.classList.remove('screen-tear'),300); }
  ...
  function globalBreak(){ const a=document.querySelector('.screen.active'); if(!a) return;
    a.classList.add('global-break'); rollBarOnce(); rgbSplitPulse(); screenTearOnce(); flashNoise(340);
    setTimeout(()=> a.classList.remove('global-break'),360);
  }
  (function jitterLoop(){
    const next = 1500 + Math.random()*3000;
    setTimeout(()=>{ const a=document.querySelector(".screen.active");
      if(a){ a.classList.add("jolt"); setTimeout(()=>a.classList.remove("jolt"),220); }
      if(Math.random()<0.25) rollBarOnce(); if(Math.random()<0.20) rgbSplitPulse(); jitterLoop();
    }, next);
  })();
  ...
  function startInitialLoading(done){
    let p=0; (function step(){
      p += (p<40?1:(p<80?2:5)); if(p>100) p=100;
      perEl.textContent = p+"%";
      if(p<100){ setTimeout(step, Math.max(18, 120*Math.pow(0.85,p/10))); }
      else{ scrLoading.style.transition="opacity .6s"; scrLoading.style.opacity="0"; setTimeout(()=>{ scrLoading.style.opacity=""; done&&done(); }, 620); }
    })();
  }
  ...
  function startStorySequence(){
    // 1) 本文ウィンドウを下から差し込み
    winMain.classList.add('win-insert');
    // 2) 少し待ってから本文をフェードイン（タイプライターなし）
    storyText.innerHTML = storyParagraphs.map(p=>`<p>${p}</p>`).join("");
    setTimeout(()=>{ storyScroll.classList.remove('content-hidden'); storyScroll.classList.add('content-show'); }, 420);

    // 3) さらに遅れて“たすけて”ウィンドウも差し込み、文字はタイプライター
    setTimeout(()=>{
      winChant.classList.remove('hidden');
      winChant.classList.add('win-insert');
      setTimeout(()=> startChant(), 520);
    }, 1200);
  }
  ...
  function typeText(el, text, speed=20){
    return new Promise(resolve=>{
      let i=0; (function step(){
        ...
        el.textContent += text.charAt(i++); const delay = Math.max(8, speed + (Math.random()*10-5));
        chantWrap.scrollTop = chantWrap.scrollHeight; setTimeout(step, delay);
      })();
    });
  }
  function startChant(){
  ...
  function startRecordLoading(){
    ...
    let p=0; const t=setInterval(()=>{
      ...
      if(p>=100){ clearInterval(t); setTimeout(()=>{ globalBreak(); showScreen(scrRecords); buildRecords();
        setTimeout(triggerAutoUpdate, 1000 + Math.random()*1200); setTimeout(slowCorruptRecords, 1500);
      },200);}
    },90);
  }
  ...
  /* ------- カルテ生成 ------- */
  const recordsData = [
    { no:"314", name:"<span class='mosaic'>████</span>", diag:"B-33型《バーバラさん》接触症", sym:"頭痛、悪夢、窓の外の気配", prog:"赤い日記帳が届いた夜から症状出現。夜間、窓から赤い日記が持ち去られるのを確認。", mosaic:true },
    { no:"052", name:"██", diag:"窓辺幻視症", sym:"吐き気、視線恐怖", prog:"窓辺で赤い日記を確認。以後、睡眠障害が悪化。", mosaic:false },
    { no:"208", name:"<span class='mosaic'>██</span>", diag:"接近過敏", sym:"首筋の寒気、肩圧迫感", prog:"夜半、背部に重量感。窓を開けると軽減。", mosaic:true },
    { no:"666", name:"<span class='mosaic'>███</span>", diag:"呼称追跡性障害", sym:"耳鳴り、名を呼ばれる幻聴", prog:"呼ばれた翌日、窓に赤い日記が置かれていた。", mosaic:true },
    { no:"094", name:"█", diag:"夜間接触症候群", sym:"過度の不安、失神", prog:"窓を開けた際、背中に何かを感じ赤い日記が消失。体調急変。", mosaic:false },
    { no:"271", name:"<span class='mosaic'>████</span>", diag:"視覚残像症", sym:"視覚異常、残像", prog:"赤い日記帳の残像を複数回確認。頭痛増悪。", mosaic:true },
    { no:"439", name:"██", diag:"急性幻視性不安", sym:"視覚異常、記憶欠落", prog:"証言と記録の齟齬が多発。最後に赤い日記帳を所持。", mosaic:false }
  ];
  ...
  function corruptString(src,rate){ let out=""; for(let i=0;i<src.length;i++){ const ch=src[i]; if(/\s/.test(ch)){out+=ch;continue;}
  ...
  function applyCorruption(s,rate){ s.forEach(it=> it.node.nodeValue = corruptString(it.original, rate)); }
  function restoreCorruption(s){ s.forEach(it=> it.node.nodeValue = it.original); }
  function startModalCorruption(root,{startRate=.03,step=.035,maxRate=.32,periodMs=1100}={}){
    stopModalCorruption(); corruptSession = collectTextNodes(root).map(n=>({node:n,original:n.nodeValue}));
    applyCorruption(corruptSession,startRate);
    corruptTimer=setInterval(()=>{ startRate=Math.min(maxRate,startRate+step); applyCorruption(corruptSession,startRate);
      root.closest('.modal-inner')?.classList.add('corrupt-flash'); setTimeout(()=>root.closest('.modal-inner')?.classList.remove('corrupt-flash'),180);
    },periodMs);
  }
  function stopModalCorruption(){ if(corruptTimer){clearInterval(corruptTimer);corruptTimer=null;} if(corruptSession.length){restoreCorruption(corruptSession);corruptSession=[];} }
  ...
  modalClose.addEventListener("click", ()=>{ stopModalCorruption(); modal.classList.remove("open"); modal.setAttribute("aria-hidden","true"); });

  /* ------- 自動更新・腐食 ------- */
  let autoUpdatedOnce=false;
  function triggerAutoUpdate(){
    if(autoUpdatedOnce) return; autoUpdatedOnce=true;
    const cards=Array.from(document.querySelectorAll("#records-list .card")); if(cards.length<=1) return;
    const target=cards[1 + Math.floor(Math.random()*(cards.length-1))];
    const field=target.querySelectorAll("td"); if(!field.length) return;
    const last=field[field.length-1]; last.innerHTML=`状態悪化を確認。<span class="mosaic">赤い日記帳</span>の所在不明。<span class="badge-update">更新</span>`;
  ...
  function slowCorruptRecords(){
    ...
    fields.forEach((td,i)=>{ const nodes=collectTextNodes(td).map(n=>({node:n,original:n.nodeValue}));
      let t=0; setInterval(()=>{ t++; const r=Math.min(0.22, 0.02+0.02*t); nodes.forEach(it=> it.node.nodeValue = corruptString(it.original, r)); }, 4000 + i*1200);
    });
  }

  /* ------- ループFX ------- */
  (function randomFxLoop(){
    const c=Math.random(); if(c<0.30) rollBarOnce(); else if(c<0.55) rgbSplitPulse(); else if(c<0.75) screenTearOnce(); else flashNoise(220);
    if(Math.random()<0.25){ const a=document.querySelector('.screen.active'); a?.classList.add('vsync-drift'); setTimeout(()=> a?.classList.remove('vsync-drift'), 5000 + Math.random()*4000); }
